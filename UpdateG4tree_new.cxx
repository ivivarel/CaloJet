...
bool UpdateG4tree(TString G4file, TString Truthfile, TString Newfile="output.root", TString G4treename = "B4", TString Truthtreename = "truth", TString Newtreename = "B4merge" )
{
  ...
  T_new->Branch("Energyem", &Energyem);
  T_new->Branch("EnergyScin", &EnergyScin);
  T_new->Branch("EnergyCher", &EnergyCher);
  T_new->Branch("NofCherenkovDetected", &NofCherenkovDetected);
  T_new->Branch("EnergyTot", &EnergyTot);
  T_new->Branch("PrimaryParticleEnergy", &PrimaryParticleEnergy);
  T_new->Branch("PrimaryParticleName", PrimaryParticleName);
  T_new->Branch("neutrinoleakage", &neutrinoleakage);
  T_new->Branch("leakage", &leakage);
  ...
  T->Branch("mcs_n", &mcs_n);          
  T->Branch("mcs_E", &mcs_E);
  T->Branch("mcs_pt", &mcs_pt);
  T->Branch("mcs_m", &mcs_m);
  T->Branch("mcs_eta", &mcs_eta);
  T->Branch("mcs_phi", &mcs_phi);
  T->Branch("mcs_status", &mcs_status);
  T->Branch("mcs_barcode", &mcs_barcode);
  T->Branch("mcs_pdgId", &mcs_pdgId);
  T->Branch("mcs_charge", &mcs_charge);
  T->Branch("mcs_vx_x", &mcs_vx_x);
  T->Branch("mcs_vx_y", &mcs_vx_y);
  T->Branch("mcs_vx_z", &mcs_vx_z);
